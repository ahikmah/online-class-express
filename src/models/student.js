...
const getMyClassByIdUser = (idUser) => {
    const qs = `SELECT 
	u.full_name student_name, c.name course_name, SUM(if(p.score > 0, 1, 0))/ COUNT(p.score)*100  AS progress_in_percent, CASE WHEN (SUM(if(p.score > 0, 1, 0))/ COUNT(p.score)*100) = 100 THEN 'completed' ELSE 'ongoing' END AS "status", SUM(if(p.score > 0, p.score, 0))/ SUM(if(p.score > 0, 1, 0)) AS score FROM users u JOIN student_course sc ON u.id = sc.student_id JOIN student_chapter_progress p ON sc.id = p.student_course_id JOIN course_chapters cc ON cc.id = p.course_chapter_id JOIN courses c ON cc.courses_id = c.id WHERE u.id=? GROUP BY c.name`;
